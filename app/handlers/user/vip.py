...
async def choose_bill(call: types.CallbackQuery, session: AsyncSession, user: User):
...
async def pre_balance_bill(call: types.CallbackQuery, session: AsyncSession, user: User):
    ...
    await call.message.edit_text(
        texts.user.BUY_VIP_BALANCE % (item['days'], item['price'], user.balance),
        reply_markup=nav.inline.confirm_buy_balance(item_id),
    )


async def balance_bill(call: types.CallbackQuery, session: AsyncSession, user: User):
    ...
    if user.balance < 0:
        ...
        return await call.answer('💰 Недостаточно средств, пожалуйста, пополните баланс', True)
...
def register(router: Router):
    ...
    router.message.register(vip_menu, Text(['Поиск М 👨', 
                                            'Поиск Ж 👩', 
                                            'Пошлый чат 🔞', 
                                            'Поиск по полу ♂️',
                                            'Комнаты 🏠']), IsVip(False))
    ...
    router.callback_query.register(pre_balance_bill, Text(startswith='buy:balance'))
    router.callback_query.register(balance_bill, Text(startswith='accept:buy:balance'))
