...
async def pre_top_up_balance(call: types.CallbackQuery, state: FSMContext, session: AsyncSession, user: User):
    with suppress(TelegramAPIError):

        await call.message.edit_text('<i>На какую сумму вы хотите пополнить баланс?</i>')
    
    await state.set_state('add.balance')



async def top_up_balance(message: types.Message, state: FSMContext, session: AsyncSession, 
                         user: User, payment: BasePayment):

    try: amount: int = int(message.text)
    except ValueError:
...
async def back_bill(call: types.CallbackQuery, user: User):
...
def register(router: Router):
    ...
    router.callback_query.register(check_bill, Text(startswith='check:profile:'))
